...
<head>
  ...
  body {
	...
	overflow: hidden;
	/* Evita el scroll mientras aparece la pantalla de carga */
}
...
#map {
  width: 100%;
  height: 100vh; /* Ensure the height is set to 100% of the viewport */
  top: 0;
  left: 0;
}
...
.list-container {
	...
	overflow-y: auto;
	display: none;
	/* Ocultamos el listado por defecto */
	flex-direction: column;
	z-index: 1200;
}
...
.legend-panel {
	position: absolute;
	bottom: 70px;
	/* Asegura que no se superponga con el botón */
	right: 20px;
	background: white;
	padding: 15px;
	border-radius: 8px;
...
.leaflet-control-layers {
	...
	/* Mueve el control 40px hacia arriba desde la parte inferior */
	right: 01px !important;
	/* Mueve el control 20px hacia la izquierda desde la parte derecha */
}
...
#toggleInactiveRadarsButton {
	...
	/* Animación para cambios de color y al presionar */
}
...
header {
	...
	/* Asegura que los elementos se distribuyan bien */
	align-items: center;
	width: 100%;
	flex-wrap: wrap;
	z-index: 10000;
	/* Permite que los elementos se ajusten a pantallas pequeñas */
}

.header-container {
	display: flex;
	justify-content: space-between;
	/* Los elementos se distribuyen */
	width: 100%;
	/* Asegura que el header ocupe el 100% */
}
...
.menu {
	...
	flex-wrap: wrap;
	/* Permite que los elementos se ajusten si no hay suficiente espacio */
}

.menu li {
	margin-right: 15px;
	margin-bottom: 10px;
	/* Añade un margen inferior para los elementos en pantallas pequeñas */
}


/* General para los botones */
...
.map-button {
	...
	white-space: nowrap;
	/* Evita que el texto de los botones se divida */
}
  </style>
</head>
<body>
...
<header>
  <div class="header-container">
    ...
    <nav>
      <ul class="menu">
        <!-- Botón de Mostrar radares en listado -->
       <li><button class="toggle-button" id="toggle-button">Mostrar radares en listado</button></li>
        
        <!-- Enlace de ADMIN -->
        <li><a href="/login" class="buttonLogin"><b>INICIAR SESIÓN</b></a></li>
        
        <!-- Botón para añadir radares -->
        <li><button id="addRadarButton" class="map-button"><b>+</b></button></li>
        
        <!-- Botón de ayuda (si es necesario) -->
        <li><button id="helpButton2" class="help-button"><b>?</b></button></li>
      </ul>
    </nav>
  </div>
</header>
  ...
  inactiveSection.style.display =
    ...
    toggleButton.addEventListener('click', () => {
      if (listDiv.style.display === 'none' || listDiv.style.display === '') {
        ...
        toggleButton.textContent = 'Mostrar radares en listado';
      }
    });
  </script>
